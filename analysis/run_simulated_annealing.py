...
def random_neighbouring_tree(T,G):
    ...
    # find the edge to reconnect the components from the original graph
    edges = []
    for u in component1:
        for v in component2:
...
def simulated_annealing(initial_tree, iterations, graph):
    ...
    for _ in range(iterations):
        ...
        random_neighbouring_tree(new_tree, graph)
...
for file_prefix in ['complete_grid_graph_', 'incomplete_grid_graph_', 'random_d_regular_graph_', 'random_graph_', 'scale_free_graph_']:
    for i in range(1,11):
        ...
        assert is_tree(initial_tree) == True

        iterations = 1000
        result_tree = simulated_annealing(initial_tree, iterations, graph)

        assert is_tree(result_tree) == True

        end_time=time.time()

        resulting_graph = result_tree.to_networkx()

        print(f'{algorithm},{file_path},{resulting_graph.number_of_nodes()},{count_leaf_nodes(result_tree)},{end_time-start_time}')
